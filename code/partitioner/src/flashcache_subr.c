...
flashcache_invalid_get(struct cache_c *dmc, int set)
{
	...
	if (index == FLASHCACHE_NULL)
		return -1;
//	if(container_occupied[container] > container_size[container])
//			return -1;
	index += (set * dmc->assoc);
...
flashcache_invalid_insert(struct cache_c *dmc, int index)
{
	...
	//printk("index : %d set:%d start_index:%d set_ix:%d, cacheblk addr:%llx, next:%d, prev :%d", index, set, start_index, set_ix, cacheblk, cacheblk->hash_next, cacheblk->hash_prev);
	//printk("------------------------------\n\n");
}
...
{
	struct kcached_job *job;
	//printk("inside kcacked job\n");
	job = flashcache_alloc_cache_job();
	//printk("flashcache alloc job");
	...
	if(bio== NULL) {//if (index != -1) { // if in cache then assigne cache s sector and size
