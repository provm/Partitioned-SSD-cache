...
int flashcache_change_container_size(int containerID, int val);
...
struct flashcache_stats {
	...
	unsigned long container_size[NUM_CONTAINERS];
	unsigned long container_occupied[NUM_CONTAINERS];
	unsigned long container_blocks_requested[NUM_CONTAINERS];
};
...
void flashcache_free_pending_jobs(struct cache_c *dmc, struct cacheblock *cacheblk,  int error, int container);
...
void flashcache_clean_set(struct cache_c *dmc, int set, int force_clean_blocks, int container);
void flashcache_sync_all(struct cache_c *dmc);
void flashcache_reclaim_fifo_get_old_block(struct cache_c *dmc, int start_index, int *index, int container);
...
void flashcache_merge_writes(struct cache_c *dmc, 
			     ...
			     int *nr_writes, int set, int container);
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,26)
...
void flashcache_detect_fallow(struct cache_c *dmc, int index, int container);
void flashcache_clear_fallow(struct cache_c *dmc, int index, int container);
...
void flashcache_hash_remove(struct cache_c *dmc, int index, int container);
int flashcache_hash_lookup(struct cache_c *dmc, int set,  sector_t dbn, int container);
void flashcache_hash_insert(struct cache_c *dmc, int index, int container);

void flashcache_invalid_insert(struct cache_c *dmc, int index,int container);
void flashcache_invalid_remove(struct cache_c *dmc, int index, int container);
int flashcache_invalid_get(struct cache_c *dmc, int set, int container);
...
void flashcache_copy_data(struct cache_c *dmc, struct cache_set *cache_set,
			  int nr_writes, struct dbn_index_pair *writes_list, int container);
