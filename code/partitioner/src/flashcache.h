...
#define DMC_PREFIX "Cntr-flashcache: "
...
//extern unsigned long container_size[NUM_CONTAINERS];
//extern unsigned long container_occupied[NUM_CONTAINERS];
//extern unsigned long container_blocks_requested[NUM_CONTAINERS];
struct cacheblock;
...
struct container_eviction {
	struct container_eviction_list list; //4
	int type;
	int count;
	const struct eviction_operations *fops; // 4
	spinlock_t set_spin_lock;
	
};

struct eviction_operations {
	 int (*init)(struct container_eviction_list *);
	 int (*insert)( struct container_eviction_list *, struct container_eviction_list *);
	 struct container_eviction_list *  (*del) (struct container_eviction_list *);
	 struct container_eviction_list *  (* getnode) (struct container_eviction_list *);
	 void (*update) ( struct container_eviction_list *, struct container_eviction_list *);
};
struct unused {
        sector_t cbn;
        struct list_head next;
...
#define LRU 1
#define FIFO 2
/*Global unused cache blocks list*/
extern struct  unused unusedblocks;
/*Global lookup (hash)*/
extern struct rb_root lookup_tree[NUM];
extern int NUM_CONTAINERS;
/*per container meta data*/
/*Container DS for eviction of disk block numbers (list)*/
extern struct container_eviction eviction[NUM];
extern int container_size[NUM];
extern int container_occupied[NUM];
extern int container_requests[NUM];
extern int container_hits[NUM];
extern int container_misses[NUM];
 	
// reclaimation functions
int fifo_init (struct container_eviction_list *) ;
int fifo_insert ( struct container_eviction_list *,  struct container_eviction_list *);
struct container_eviction_list * fifo_del (struct container_eviction_list *);
struct container_eviction_list *   fifo_get (struct container_eviction_list *) ;
void fifo_update( struct container_eviction_list *, struct container_eviction_list *);

int lru_init (struct container_eviction_list *) ;
int lru_insert ( struct container_eviction_list *,  struct container_eviction_list *);
struct container_eviction_list * lru_del (struct container_eviction_list *);
struct container_eviction_list *   lru_get (struct container_eviction_list *) ;
void lru_update( struct container_eviction_list *, struct container_eviction_list *);
...
enum {
	...
	FLASHCACHE_WRITE_AROUND=3, // similar to write bypass
};
...
//ADDED
//#define CLEAN			0x0080 /*valid and clean (similar to valid?? but here explicitly mentioning that its clean)*/
/*
