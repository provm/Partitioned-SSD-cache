...
flashcache_writeback_md_store(struct cache_c *dmc)
{
	...
	if (sectors_expected != sectors_written) {
		DPRINTK("flashcache_writeback_md_store" "Sector Mismatch ! sectors_expected=%d, sectors_written=%d\n",
...
flashcache_writeback_create(struct cache_c *dmc, int force)
{
	...
	if (dmc->size % MD_SLOTS_PER_BLOCK(dmc))
		sectors_expected += MD_SECTORS_PER_BLOCK(dmc);
	if (sectors_expected != sectors_written) {
		DPRINTK("flashcache_writeback_create" "Sector Mismatch ! sectors_expected=%d, sectors_written=%d\n",
		       sectors_expected, sectors_written);
	...
	DPRINTK("flashcache-dbg: cachedev check - %s %s\n", header->cache_devname,
				dmc->dm_vdevname);
...
flashcache_writeback_load(struct cache_c *dmc)
{
	...
	if (header->cache_sb_state == CACHE_MD_STATE_DIRTY) {
		...
		DPRINTK(KERN_ALERT "Only DIRTY blocks exist in cache\n");
		clean_shutdown = 0;
	} else if (header->cache_sb_state == CACHE_MD_STATE_CLEAN) {
		DMINFO("Slow (clean) Shutdown Detected");
		DPRINTK(KERN_ALERT "Only CLEAN blocks exist in cache\n");
		clean_shutdown = 1;
	} else {
		DMINFO("Fast (clean) Shutdown Detected");
		DPRINTK(KERN_ALERT "Both CLEAN and DIRTY blocks exist in cache\n");
		clean_shutdown = 1;
	}
	...
	if (sectors_expected != sectors_read) {
		DPRINTK("flashcache_writeback_load" "Sector Mismatch ! sectors_expected=%d, sectors_read=%d\n",
 ...
 *  arg[3]: cache mode (from flashcache.h) // we can ignore [X]
 *  arg[4]: cache persistence (if set, cache conf is loaded from disk) [X]
 * Cache configuration parameters (if not set, default values are used.[X]
 *  arg[5]: cache block size (in sectors)
 *  arg[6]: cache size (in blocks)
 *  arg[7]: cache associativity[X]
 *  arg[8]: md block size (in sectors)
 */
 //ADDED
 // eviction operations added
static struct eviction_operations fifo_operations = {
 	.init = fifo_init,
 	.insert = fifo_insert,
 	.del = fifo_del,
 	.getnode = fifo_get, 
 ...
 static struct eviction_operations lru_operations = {
 	.init = lru_init,
 	.insert = lru_insert,
 	.del = lru_del,
 	.getnode = lru_get, 
  ...
  struct  unused unusedblocks;
/*Global lookup (hash)*/
 struct rb_root lookup_tree[NUM];

/*per container meta data*/
/*Container DS for eviction of disk block numbers (list)*/
int NUM_CONTAINERS=1;
 struct container_eviction eviction[NUM];
 int container_size[NUM];
 int container_occupied[NUM];	
 int container_requests[NUM];
 int container_hits[NUM];
 int container_misses[NUM];
 
int flashcache_ctr(struct dm_target *ti, unsigned int argc, char **argv) // called when dmsetup create executes
{
	struct cache_c *dmc;
	sector_t i, order;
	int r = -EINVAL;
	int persistence = 0;
	int j=0;
	int c1=10000,c2=30000,c3=0,c4=0,c5=0,c6=0;
	if (argc < 3) {
	...
	//ADDED
	//printk("ARGC:  %d\n",argc);
	if (sscanf(argv[11], "%d", &NUM_CONTAINERS) != 1) {
		ti->error = "flashcache: Number of containers not defined";
		goto bad3;
	}
	if (sscanf(argv[12], "%d", &c1) != 1) {
		ti->error = "flashcache: Size of containers not defined";
		goto bad3;
	}if (sscanf(argv[13], "%d", &c2) != 1) {
		ti->error = "flashcache: Size of containers not defined";
		goto bad3;
	}if (sscanf(argv[14], "%d", &c3) != 1) {
		ti->error = "flashcache:Size of containers not defined";
		goto bad3;
	}if (sscanf(argv[15], "%d", &c4) != 1) {
		ti->error = "flashcache: Sizeof containers not defined";
		goto bad3;
	}if (sscanf(argv[16], "%d", &c5) != 1) {
		ti->error = "flashcache: Size of containers not defined";
		goto bad3;
	}if (sscanf(argv[17], "%d", &c6) != 1) {
		ti->error = "flashcache: Size of containers not defined";
		goto bad3;
	}
//	printk("NUM:::----------> %s %d\n\n",argv[11], NUM_CONTAINERS);
	argc = argc - 6;
	...
	if (argc >= 8) {
		...
		if (sscanf(argv[7], "%u", &dmc->assoc) != 1) {
			ti->error = "flashcache: Invalid cache associativityss";
			r = -EINVAL;
			goto bad3;
		}
		if (!dmc->assoc || (dmc->assoc & (dmc->assoc - 1)) ||
		    ...
		    dmc->size < dmc->assoc) {
			ti->error = "flashcache: Invalid cache associativitysss";
			r = -EINVAL;
			goto bad3;
		}
	}
	...
	if (argc >= 9) {
		...
		if ((dmc->disk_assoc > 0) &&
			...
			DPRINTK(KERN_ERR "Invalid Disk Assoc assoc %d disk_assoc %d size %ld\n",
	...
	if (argc >= 10) {
		...
		if ((dmc->write_only_cache == 1) &&
			...
			DPRINTK(KERN_ERR "Write Cache Setting only valid with WRITE_BACK %d\n",
		...
		if (dmc->write_only_cache < 0 || dmc->write_only_cache > 1) {
			DPRINTK(KERN_ERR "Invalid Write Cache Setting %d\n",
			       dmc->write_only_cache);
			ti->error = "flashcache: Invalid Write Cache Setting";
			r = -EINVAL;
			goto bad3;
...
init:
	...
	//ADDED
	DPRINTK("initialise list head\n");

	INIT_LIST_HEAD(&unusedblocks.next);
	
	
	for( j=256; j<dmc->size; j++) {

		struct unused *tmp=(struct unused *)kmalloc(sizeof(struct unused),GFP_KERNEL);
		tmp->cbn = j;
		list_add_tail(&(tmp->next), &(unusedblocks.next));
	}
	printk("initalise for containers: %d %d\n", dmc->size, NUM_CONTAINERS);
	container_size[0] = c1;
	container_size[1] = c2;
	container_size[2] = c3;
	container_size[3] = c4;
	...
	for (j = 0; j<NUM_CONTAINERS; j++) { 
		container_occupied[j]= 0;
		container_requests[j] = 0;
		container_hits[j] = 0;		
		container_misses[j] = 0;
		spin_lock_init(&eviction[j].set_spin_lock);
	}
		// init eviction type as well
	/*eviction[1].type = FIFO;
	eviction[1].fops = &fifo_operations;
	eviction[1].count = 0;
	eviction[2].type = FIFO;
	eviction[2].fops = &fifo_operations;
	eviction[2].count = 0;
	
	eviction[0].fops->init(&eviction[0].list);
	eviction[1].fops->init(&eviction[1].list);
	eviction[2].fops->init(&eviction[2].list);*/
	
	DPRINTK("initialise rb tree\n");
	for(j=0; j<NUM_CONTAINERS; j++) {
		eviction[j].type = FIFO;
		eviction[j].fops = &fifo_operations;
		eviction[j].fops->init(&eviction[j].list);
		eviction[j].count = 0;
	
		lookup_tree[j]=RB_ROOT;
	}
...
flashcache_sync_for_remove(struct cache_c *dmc)
{
	do {
		...
		if (!dmc->sysctl_fast_remove) {
			 ...
			 * to finish.
			 */
			DPRINTK(KERN_ALERT "Cleaning %d blocks please WAIT\n", atomic_read(&dmc->nr_dirty));
			...
			DPRINTK(KERN_ALERT "Fast flashcache remove Skipping cleaning of %d blocks\n", 
			       atomic_read(&dmc->nr_dirty));
		}
		/* 
...
/*********** Extra job for intialising *******************/
	...
	r = dm_register_target(&flashcache_target); // this the main function call for initialasation [called when module is inserted]
	if (r < 0) {
		DMERR("cache: register failed %d", r);
	}

        DPRINTK("flashcache: %s initialized\n", flashcache_sw_version);
