...
static void flashcache_read_miss(struct cache_c *dmc, struct bio* bio,
				 int index, ktime_t kt);
static void flashcache_write(struct cache_c *dmc, struct bio* bio, ktime_t kt);
...
//dm_io_async_bvec(1, &job->job_io_regions.disk, READ,bio,flashcache_io_callback, job);
unsigned long flashcache_get_inode(struct bio *bio) {
	int retval = 0;
	if(bio == NULL)
		goto out;
	...
	if(bio->bi_io_vec->bv_page->mapping->host == NULL)
		goto out;
	retval = bio->bi_io_vec->bv_page->mapping->host->i_ino;

out :
	return retval;
}		
int flashcache_get_containerID(struct bio *bio)
{
        int retval = -EINVAL;
        struct cgroup *cgp;
        const char *name;
        ...
        //DPRINTK("Container:%s, bio sector number:%lx \n", cgp->kn->name,bio->bi_sector);
         name = cgp->kn->name;
         DPRINTK("Got a call from container%s\n",name);
         if(strcmp(name,"test_container3")==0)
                                retval = 1;
          			...
          			retval = 1;
                                ...
                                retval = 4;
		else if (strcmp(name,"test_container5")==0)
			retval=3;
         else
         			retval = 0;
  out:      
...
struct my_lookup_block *my_rb_search(struct rb_root *root, sector_t dbn) {
    //    DPRINTK("inside rb search: %lu\n",dbn);
	struct rb_node *node = root->rb_node;  /* top of the tree */

        while (node)
        {//DPRINTK("before container of\n");
            struct my_lookup_block *my_node = container_of(node, struct my_lookup_block, node);
	//DPRINTK("after container of\n");
            if (my_node->dbn > dbn)
                node = node->rb_left;
            else if (my_node->dbn < dbn)
                node = node->rb_right;
            else
                return my_node;  /* Found it */
        }
        return NULL;
}
/*implicit declaration of rb delte node*/
//void rb_erase(struct rb_node *victim, struct rb_root *tree);

int dm_io_async_bvec(unsigned int num_regions,struct dm_io_region *where,  int rw,  struct bio *bio,    io_notify_fn fn,    void *context)
{
	struct dm_io_request iorq;
	int x;
//	 ktime_t k1,k2;
//	 struct kcached_job *job = (struct kcached_job *) context;
	...
	// ADDED TIME
	// third arg : i can get read or write request
	// need to find action type : last arg : job->action :compare
	x = dm_io(&iorq, num_regions, where, NULL);
	//k2 = ktime_get();
//	printk("time for %d for action %d : %ld\n",rw, job->action,  ktime_to_ns(ktime_sub(k2, k1) ));
//printk("%d %ld\n", job->action,  ktime_to_ns(ktime_sub(k2, k1) ));
	return x;
}
...
//	unsigned long flags;
	...
	//DPRINTK("in IO call back\n");
	VERIFY(index != -1);		
	bio = job->bio;
	VERIFY(bio != NULL);
	if (unlikely(error)) {
		error = -EIO;
		DPRINTK("flashcache_io_callback: io error %ld block %lu action %d\n", 
		      error, job->job_io_regions.disk.sector, job->action);
		if (!dmc->bypass_cache && dmc->cache_mode != FLASHCACHE_WRITE_BACK) {
			DPRINTK("flashcache_io_callback: switching %s to BYPASS mode",
			      dmc->cache_devname);
			dmc->bypass_cache = 1;
		}
	}
	...
	switch (job->action) {
	case READDISK:
		//printk("flashcache_io_callback: READDISK  %d\n",	index);
		...
		if (likely(error == 0)) {
			...
			kt1 = job->k1;
			kt2 =  ktime_get();
			/*if(flashcache_get_containerID(bio) == 1) {
		...
		break;
	case READCACHE:
	//	printk("flashcache_io_callback: READCACHE %d\n",
	//		index);
		//spin_lock_irqsave(&cache_set->set_spin_lock, flags);
		kt1 = job->k1;
		kt2 =  ktime_get();
		...
		*/	
		...
		if (likely(error == 0)) {
			if (flashcache_validate_checksum(job)) {
				DPRINTK("flashcache_io_callback: Checksum mismatch at disk offset %lu", 
		...
		break;		       
	case READFILL:
//		printk("flashcache_io_callback: READFILL %d\n",index);
		//spin_lock_irqsave(&cache_set->set_spin_lock, flags);
		kt1 = job->k1;
		kt2 =  ktime_get();
		//printk("READFILL %ld\n", ktime_to_ns(ktime_sub(kt2, kt1) ));
...
flashcache_do_pending_error(struct kcached_job *job)
{
	...
	if (bio != NULL) {
		...
		/*if (!dmc->bypass_cache)  // suppress massive console output 
			DPRINTK("flashcache_do_pending_error: Re-launching errored IO"
...
flashcache_do_pending_noerror(struct kcached_job *job)
{
	...
	DPRINTK("flashcache_do_pending: Index %d %x",
	...
	while (freelist != NULL) {
		...
		DPRINTK("flashcache_do_pending: Sending down IO %lu",
			(unsigned long)pending_job->bio->bi_sector);
		/* Start uncached IO */
...
hash_block(struct cache_c *dmc, sector_t dbn)
{
	...
	//DPRINTK("Hash: dbn %lu value:(%lu) set:%lu \n", dbn, value, set_number);
	return set_number;
}
...
static int  flashcache_lookup(struct cache_c *dmc, struct bio *bio, int *cache_block) {

	int res = 0;
	struct unused *tmp;
	struct container_eviction_list *refered_block;
	struct my_lookup_block *new;
	struct list_head *pos,*q;
	sector_t block = bio->bi_sector;
	struct my_lookup_block *search_result;
	//ktime_t k1,k2;
	int containerID = flashcache_get_containerID(bio);
	 ktime_t k1,k2;
//	unsigned long inode = flashcache_get_inode(bio);
	*cache_block = 0;
	if(containerID < 0){
	      printk(KERN_INFO "Invalid container ID = %d\n",containerID);
	      return -1;
	 }     
	// BUG_ON(containerID > 1);
	 // lookup
	
	//

	 search_result = my_rb_search(&lookup_tree[containerID], block);
	//
	
	container_requests[containerID]++;
	//cache MISS    
	if (search_result == NULL) {
	//	printk("%s Cache miss :%d \n",__func__,containerID);
		container_misses[containerID]++;
	//	printk("MISS:%lu size:%d\n", inode, container_occupied[containerID]);			
		if (container_occupied[containerID] < container_size[containerID]) 
		{
			//printk("cache miss size not exceeded\n");
			// get new node from unused list
			list_for_each_entry(tmp, &unusedblocks.next, next) {
				//DPRINTK("unused%lu\n",tmp->cbn);
				*cache_block = tmp->cbn;
				break;
			}
			spin_lock(&eviction[containerID].set_spin_lock);
			// delete new node from unused list
			list_for_each_safe(pos,q,&unusedblocks.next) {
				tmp = list_entry(pos, struct unused, next);
			//	DPRINTK("from unsued list: %lu\n",tmp->cbn);
				list_del(pos);
				break;
			}
			
                	container_occupied[containerID]++;
                	// new eviction node
                	//refered_block = eviction[containerID].fops->insert();
			refered_block = (struct container_eviction_list*) kmalloc(sizeof(struct container_eviction_list), GFP_KERNEL);
			// new rb tree node
			new = (struct my_lookup_block*) kmalloc(sizeof(struct my_lookup_block), GFP_KERNEL);
			
			
                	refered_block->dbn = block;
                	refered_block->cbn = *cache_block;
			refered_block->ptr = (struct my_lookup_block*)new;
			
			// add eviction node to FIFO
			//DPRINTK("add node to FIFO list\n");
			//printk("container id: %d\n",containerID);
			eviction[containerID].fops->insert(&(eviction[containerID].list), refered_block);
                	//list_add_tail(&(refered_block->next), &(eviction[containerID].next));

			// add new node to rb tree
		//	DPRINTK("add node to rb tree\n");
			new->dbn=block;
			new->cbn=*cache_block;
			new->ptr=(struct container_eviction_list *)refered_block;
			my_rb_insert(&(lookup_tree[containerID]), new);
			//DPRINTK("inserted node into tree\n");
			spin_unlock(&eviction[containerID].set_spin_lock);
			res = 0;
		}
	else {
			...
			sector_t dbn;
			k1 = ktime_get();
			eviction[containerID].count++;
			//printk("cache miss size exceeded\n");
			// get block from containers eviction FIFO list
			spin_lock(&eviction[containerID].set_spin_lock);
			refered_block = eviction[containerID].fops->del(&(eviction[containerID].list));
			if(refered_block == NULL)
				printk("FLASHCACHE ERROR : Refered block is NULL");
			*cache_block = refered_block->cbn;
			dbn = refered_block->dbn; // need for write through
			search_result = refered_block->ptr;
			if(search_result == NULL) 
				printk("FLASHCACHE ERROR : Result is NULL\n");
			if(search_result == NULL)
                                printk("ERROR: INconsistent metadata\n");
			// deleting node from rbtree
			rb_erase(&search_result->node, &(lookup_tree[containerID]));
			
			
			// creating new nodes
			//refered_block = (struct container_eviction*) kmalloc(sizeof(struct container_eviction), GFP_KERNEL);
                        //new = (struct my_lookup_block*) kmalloc(sizeof(struct my_lookup_block), GFP_KERNEL);
                         new = search_result;
			// adding entry to end of FIFO
			refered_block->dbn = block;
                        refered_block->cbn = *cache_block;
			refered_block->ptr = (struct my_lookup_block*)new;
			eviction[containerID].fops->insert(&(eviction[containerID].list), refered_block);
                   
			// search rb node with key = dbn
			// no need since kept ptr to it in FIFO	
			//	search_result = my_rb_search(&lookup_tree,dbn); 
			// removing a node from rb tree
			// adding new node(same cbn and different dbn) to rb tree
                        new->dbn=block;
                        new->cbn=*cache_block;
                        new->ptr=(struct container_eviction_list *)refered_block;
                        my_rb_insert(&(lookup_tree[containerID]), new);
			spin_unlock(&eviction[containerID].set_spin_lock);
			k2 = ktime_get();
			//printk("evict %ld\n",  ktime_to_ns(ktime_sub(k2, k1) ));
		}
	} // cache HIT
	else {
		//DPRINTK("cache hit\n");
		container_hits[containerID]++;
		//printk("HIT:%lu size:%d\n",inode, container_occupied[containerID]);
		*cache_block = search_result->cbn;
		res = 1;
	//	printk("%s Hit lookup search container: %d count: %d index %d\n",__func__,containerID, container_occupied[containerID], *cache_block);
		//update the position in lru list, not required for fifo still call generic function
		refered_block = (struct container_eviction_list *) search_result->ptr;
		eviction[containerID].fops->update(&(eviction[containerID].list), refered_block);
		//DPRINTK("--------------lokkup done----------\n");
		return VALID;
	}
	//printk("%s Miss lookup search container: %d count: %d index %d\n",__func__,containerID, container_occupied[containerID], *cache_block);
//	DPRINTK("--------------lokkup done----------\n");
	return INVALID;
...
//	unsigned long inode = flashcache_get_inode(bio);
	*cache_block = 0;
	...
	//
	 ...
	 search_result = my_rb_search(&lookup_tree[xx], block);
	//
	
	container_requests[containerID]++;
	container_requests[xx]++;
	//cache MISS    
	if (search_result == NULL) {
	//	printk("%s Cache miss :%d \n",__func__,containerID);
		container_misses[containerID]++;
		container_misses[xx]++;
	//	printk("MISS:%lu size:%d\n", inode, container_occupied[containerID]);			
		if (container_occupied[xx] < container_size[xx]) 
		{
			//printk("cache miss size not exceeded\n");
			// get new node from unused list
			list_for_each_entry(tmp, &unusedblocks.next, next) {
			...
			spin_lock(&eviction[xx].set_spin_lock);
			// delete new node from unused list
			list_for_each_safe(pos,q,&unusedblocks.next) {
				tmp = list_entry(pos, struct unused, next);
			//	DPRINTK("from unsued list: %lu\n",tmp->cbn);
			...
			new = (struct my_lookup_block*) kmalloc(sizeof(struct my_lookup_block), GFP_KERNEL);
                	...
                	refered_block->cbn = *cache_block;
			refered_block->ptr = (struct my_lookup_block*)new;
			refered_block->id = containerID;
			// add eviction node to FIFO
			//DPRINTK("add node to FIFO list\n");
			//printk("container id: %d\n",containerID);
			eviction[xx].fops->insert(&(eviction[xx].list), refered_block);
                	//list_add_tail(&(refered_block->next), &(eviction[containerID].next));
		...
		//	DPRINTK("add node to rb tree\n");
			...
			new->cbn=*cache_block;
			new->ptr=(struct container_eviction_list *)refered_block;
			my_rb_insert(&(lookup_tree[xx]), new);
			//DPRINTK("inserted node into tree\n");
			spin_unlock(&eviction[xx].set_spin_lock);
			res = 0;
		}
	else {
			...
			// get block from containers eviction FIFO list
			spin_lock(&eviction[xx].set_spin_lock);
			refered_block = eviction[xx].fops->del(&(eviction[xx].list));
			
			if(refered_block == NULL)
				printk("FLASHCACHE ERROR : Refered block is NULL");
			
			eviction[refered_block->id].count++;
			container_occupied[refered_block->id]--;
			...
			*cache_block = refered_block->cbn;
			...
			if(search_result == NULL)
                                printk("ERROR: INconsistent metadata\n");
			// deleting node from rbtree
			rb_erase(&search_result->node, &(lookup_tree[xx]));
 ...
 */
/*static int 
flashcache_lookup1(struct cache_c *dmc, struct bio *bio, int *index)
{
	...
	//int container =flashcache_get_containerID(bio);
	//int container_sets = dmc->num_sets/NUM_CONTAINERS; 
	//set_number = set_number%container_sets;
	//if(container==1) 
	//	set_number = set_number + container_sets;
	...
	//DPRINTK("=========CONTAINER :%d==========\n",container);
	DPRINTK("Cache lookup : dbn %lu io_size:%d, set = %lu\n",dbn, io_size, set_number);
	find_valid_dbn(dmc, dbn, start_index, index);
	if (*index >= 0) {
		DPRINTK("Cache lookup HIT: Block %lu io_size:%d VALID index %d\n", dbn, io_size, *index);
	...
	if (invalid != -1) {
		DPRINTK("Cache lookup MISS (INVALID): dbn %lu io_size:%d, set = %lu, index = %d, start_index = %d\n",
			     dbn, io_size, set_number, invalid, start_index);
			//container_occupied[container]++;
		*index = invalid;
	} else if (oldest_clean != -1) {
	
	DPRINTK("Cache lookup MISS (VALID): dbn %lu io_size:%d, set = %lu, index = %d, start_index = %d\n",
			     dbn, io_size, set_number,  oldest_clean, start_index);
			     
		//DPRINTK("Cache lookup MISS (VALID): dbn %llu(%lu), set = %d, index = %d, start_index = %d",   dbn, io_size, set_number, oldest_clean, start_index);
		*index = oldest_clean;
	} else {
		DPRINTK("Cache read lookup MISS (NOROOM): dbn %lu io:%d, set = %lu\n",	dbn, io_size, set_number);
	}
...
//ADDED
// function to flush the dirty blocks of the cache 
// flush all or the index sent as function argument??
/*static void flashcache_dirty_writeback1(struct cache_c *dmc, int index) {
...
static void
flashcache_read_hit(struct cache_c *dmc, struct bio* bio, int index, ktime_t kt1)
{
	struct cacheblock *cacheblk;
...
static void
flashcache_read_miss(struct cache_c *dmc, struct bio* bio, int index, ktime_t kt1)
{
	struct kcached_job *job;
//	struct cacheblock *cacheblk = &dmc->cache[index];
//	int set = index / dmc->assoc;
...
//	}
	...
	//} else 
	{
		...
		dm_io_async_bvec(1, &job->job_io_regions.disk, READ,
				 bio,
				 flashcache_io_callback, job);
		//DPRINTK("after dm io async\n");
		//flashcache_clean_set(dmc, set,  dmc->sysctl_clean_on_read_miss);
	}
}

static void
flashcache_read(struct cache_c *dmc, struct bio *bio, ktime_t kt1)
{
	int index;
	int res;
	struct cacheblock *cacheblk;
...
//	printk("Got a %s for %lu (%u bytes)\n", (bio_rw(bio) == READ ? "READ":"READA"), bio->bi_sector, bio->bi_size);
...
//	if (ts1 !=NULL && ts2 !=NULL)
	//	printk("read lookup time: %ld\n", ts2->tv_nsec - ts1->tv_nsec );
	//if(k1 !=NULL && k2 !=NULL)
			//printk("read lookup ktime :%ld\n",  ktime_to_ns(ktime_sub(k2, k1) ));
	/* Cache Read Hit case */ //[IGNORING]
        if(res < 0){
                 //submit_bio(bio->bi_rw, bio);
                 //flashcache_start_uncached_io(dmc, bio);
                 DPRINTK(KERN_INFO "submitting bio on read invalid lookup\n");
	...
	if (res > 0) {
		...
		if ((cacheblk->cache_state & VALID) && 
			...
			flashcache_read_hit(dmc, bio, index,kt1);
			return;
		}
	}
	/*
...
//spin_lock_irqsave(&dmc->ioctl_lock, flags);
	...
	//	    (dmc->cache[index].cache_state == INVALID))
			...
			//flashcache_invalid_insert(dmc, index);
	//	flashcache_setlocks_multidrop(dmc, bio);
	//	DPRINTK("Cache read: Block %lu(%lu):%s\n",
	...
	for (i = start_index ; i < end_index ; i++) {
		...
		if ((io_start >= start_dbn && io_start < end_dbn) ||
			...
			if (!(cacheblk->cache_state & (BLOCK_IO_INPROG | DIRTY)) &&
				...
				atomic_dec(&dmc->cached_blocks);
				DPRINTK("Cache invalidate (!BUSY): Block %lu %x \n",
			      ...
			      struct pending_job *pjob)
{
	...
	if (index == -1) {
		...
		if (index != -1) {
			DPRINTK(KERN_ERR "Invalidate: Did not find block on hash "
			       "but found in set %d\n", index);
			DPRINTK(KERN_ERR "io_start = %lu bi_sector = %lu bi_end = %lu\n",
			       io_start, 
			       bio->bi_sector, 
			       bio->bi_sector + (to_sector(bio->bi_size) - 1));
			DPRINTK(KERN_ERR "cache_state = %x hash_state = %x cacheblk->dbn = %lu\n",
...
static void
flashcache_write_miss(struct cache_c *dmc, struct bio *bio, int index, ktime_t kt)
{
	struct cacheblock *cacheblk;
	struct kcached_job *job;
//	int queued;
...
//	}
	...
	//} else 
	{
		...
		/*if (dmc->cache_mode == FLASHCACHE_WRITE_BACK) {
		 ...
		 // making write through as default
		 {
			//VERIFY(dmc->cache_mode == FLASHCACHE_WRITE_THROUGH);
			
			dm_io_async_bvec(2, (struct dm_io_region *)&job->job_io_regions, WRITE, bio, flashcache_io_callback, job);
		}
		//flashcache_clean_set(dmc, set,     dmc->sysctl_clean_on_write_miss);
	}
}

static void
flashcache_write_hit(struct cache_c *dmc, struct bio *bio, int index, ktime_t kt1)
{
	struct cacheblock *cacheblk;
	struct pending_job *pjob;
	struct kcached_job *job;
	...
	DPRINTK("write hit\n");
	cacheblk = &dmc->cache[index];
	DPRINTK("cacheblk pointed\n");
	//job = new_kcached_job(dmc, bio, index);
	//DPRINTK("job returned\n");
	//if(job == NULL) {
	//	DPRINTK("job is NULL\n");
	//	return;
	//	}
	if (!(cacheblk->cache_state & BLOCK_IO_INPROG) && (cacheblk->nr_queued == 0)) {
		...
		DPRINTK("job==null\n");
			 ...
			 * pending jobs.
			 */
			DPRINTK("flashcache: Write (hit) failed ! Can't allocate memory for cache IO, block %lu\n", 
			      cacheblk->dbn);
			flashcache_bio_endio(bio, -EIO, dmc, NULL);
			DPRINTK("after bio endio\n");
			//spin_lock_irq(&cache_set->set_spin_lock);
		//	flashcache_free_pending_jobs(dmc, cacheblk, -EIO);
		//	cacheblk->cache_state &= ~(BLOCK_IO_INPROG);
			//spin_unlock_irq(&cache_set->set_spin_lock);
...
static void
flashcache_write(struct cache_c *dmc, struct bio *bio, ktime_t kt1)
{
	int index;
	int res;
	struct cacheblock *cacheblk;
//	ktime_t k1,k2;
	...
	if (res != -1) {
		...
		if ((cacheblk->cache_state & VALID) && 
		    (cacheblk->dbn == bio->bi_sector)) {
			/* Cache Hit */
			DPRINTK("flashcache_write -hit: %d\n",index);
			flashcache_write_hit(dmc, bio, index, kt1);
		} else {
			/* Cache Miss, found block to recycle */
			DPRINTK("flashcache_write miss: %d\n",index);
			flashcache_write_miss(dmc, bio, index, kt1);
		}
       }
	else {
		 DPRINTK("Invalid container ID or cache has no room\n");
		 // write to disk
		 DPRINTK(KERN_INFO "I dont want write now\n");
                 bio->bi_bdev = dmc->disk_dev->bdev;
                 generic_make_request(bio);
...
//ADDED call for parda function
if(inst_generic_make_request_ptr !=NULL) {
	(*inst_generic_make_request_ptr)(bio);
}
// this is for avoiding writes, without writes everything works good
	...
	if ( bio_data_dir (bio) == WRITE ) {
		...
		bio->bi_bdev = dmc->disk_dev->bdev;
		generic_make_request( bio);
		//   flashcache_start_uncached_io(dmc, bio);  //CRASHING
		//   submit_bio(bio->bi_rw,bio); // On a loop
		goto out;
	}
	#endif
	//DPRINTK("Map\n");
...
//	spin_unlock_irqrestore(&dmc->ioctl_lock, flags);
	...
	
	{
		...
		if (bio_data_dir(bio) == READ) {
			...
			flashcache_read(dmc, bio, kt1);
			//kt2 = ktime_get();
			//printk("total read ktime :%ld\n",  ktime_to_ns(ktime_sub(kt2, kt1) ));
			//printk("T %ld\n",  ktime_to_ns(ktime_sub(kt2, kt1) ));
		}
		else {
		//ADDED TIME
			//kt1 = ktime_get();
			flashcache_write(dmc, bio, kt1);
		//	kt2 = ktime_get();
		//	printk("T %ld\n",  ktime_to_ns(ktime_sub(kt2, kt1) ));
		}
	}
...
*/	if (likely(read_err == 0 && write_err == 0)) {
		...
		//printk(" ERROR in dm_kcopyd callback\n");
/*		if (read_err)
			read_err = -EIO;
		if (write_err)
			write_err = -EIO;
...
int flashcache_change_container_size(int containerID, int size) {
	//int i;
	//printk("CACHE %d %d %d %d\n", container_occupied[1], container_occupied[2]); //container_occupied[3], container_occupied[4] );
	/*for( i = 0; i<NUM_CONTAINERS; i++) {
		printk("---------------- CONTAINER : %d ------------------ \n", i);
		printk("Total blocks: %d\n", container_size[i]);
		printk("Container occupied: %d\n", container_occupied[i]);
		printk("Container requests: %d\n", container_requests[i]);
		printk("Container hits: %d\n", container_hits[i]);
		printk("Container misses: %d\n", container_misses[i]);
		printk("Container evictions: %d\n", eviction[i].count);
		
	
	}*/
	//return 1;
	//printk("new size: %d",size);
	int extra = 0,index=0;
	//struct kcached_job *job;
	printk("\ncontainer iD: %d, new size : %d\n",containerID, size);
	if(containerID > NUM_CONTAINERS  || size < 0) 
		return 0;
	// increasing the cache size, then no eviction needed
	if(container_size[containerID] <= size) {
		container_size[containerID] = size;
		return 1;
	} 
	extra = container_occupied[containerID] - size;
	if(extra <= 0) {
		container_size[containerID] = size;
		return 0;
	}
	// if occupied is less than given size then no eviction needed 
	while(extra--) {
	        struct container_eviction_list *refered_block;
        	//struct list_head *pos,*q;
        	struct my_lookup_block *search_result=NULL;
		struct unused *tmp;
		
		// get block from containers eviction FIFO list and delete
		//printk("getting from from FIFO \n");
		refered_block = eviction[containerID].fops->del(&(eviction[containerID].list));
		search_result = refered_block->ptr;
		index = refered_block->cbn;
		
                        
                // remove entry from FIFO and add it to end of FIFO with updated dbn
                        
            
                 if(search_result == NULL) {
                        DPRINTK("ERROR: INconsistent metadata\n");
			return 0;
		}
		
		
		//printk("deleting node from rbtree\n\n");
                rb_erase(&search_result->node, &(lookup_tree[containerID]));
		
		// add entry to free list
		tmp = (struct unused *)kmalloc(sizeof(struct unused),GFP_KERNEL);
                tmp->cbn = refered_block->cbn;
                list_add_tail(&(tmp->next), &(unusedblocks.next));

		container_occupied[containerID]--;
		//printk(" removed %lu from FIFO, now size is: %d\n",refered_block->cbn, container_occupied[containerID]);
		
		// till here only data structures are modified, next flush the data into disk
		...
		job->bio = NULL;
		job->job_io_regions.disk.sector = refered_block->dbn;
		job->job_io_regions.disk.count = dmc->block_size;
		///printk(" WRITEDISK_SYNC \n");
		...
		job->action = WRITEDISK;
	//printk("index: %d  cache sector: %llu    disk sector: %llu     block size: %llu\n", index, job->job_io_regions.cache.sector, job->job_io_regions.disk.sector, job->job_io_regions.disk.count );
...
int flashcache_get_container_status(int a) {
	int i=0;
	flashcache_change_container_size_ptr = &flashcache_change_container_size;
	for( i = 0; i<NUM_CONTAINERS; i++) {
		printk("---------------- CONTAINER : %d ------------------ \n", i);
		printk("Total blocks: %d\n", container_size[i]);
		printk("Container occupied: %d\n", container_occupied[i]);
		...
		printk("Container evictions: %d\n", eviction[i].count);
		//if(i==3) {
		container_requests[i]=0;
		container_hits[i]=0;
		container_misses[i]=0;
		eviction[i].count=0;
	//	}
...
EXPORT_SYMBOL(flashcache_change_container_size);
EXPORT_SYMBOL(flashcache_get_container_status);
